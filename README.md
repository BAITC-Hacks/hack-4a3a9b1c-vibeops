...
В примере окружения выбрана `gpt-6-sol`; с ней выполнена живая приёмка релиза `v0.1.0` ([отчёт](docs/release-acceptance.md)). `gpt-6-astra` — только явно выбираемый кандидат: для него три AI-адаптера задают `reasoning.effort=low`, но доступ и задержку нужно проверить на своём сервере. Mock-тесты адаптеров не доказывают качество или скорость настоящей модели. [План доступа жюри](docs/jury-access.md).
...
- **Продуктовый AI:** OpenAI Responses API, в примере окружения `gpt-6-sol`; структурированный ответ, `store:false`, без автоматических повторов. Модель извлекает условия и выбирает свидетельства; цены, календарь, ID и порядок контролирует код.
