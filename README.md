...
`OPENAI_API_KEY` — реальный серверный ключ с доступом к выбранной модели и Responses API. Он не входит в репозиторий, архивы релиза или клиентскую сборку. При запуске production сервер загружает `.env` сам. Переменные среды имеют приоритет. Никогда не используйте клиентский `VITE_*` для ключей.
