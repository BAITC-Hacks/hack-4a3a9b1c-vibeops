...
function amountOf(value: string, scale = ''): number {
  ...
  return Number(value.replace(/[ \u00a0\u202f]/gu, '').replace(',', '.')) * multiplier;
}
...
export function moneyMentions(input: string): MoneyMention[] {
  ...
  const put = (match: RegExpMatchArray, amount: string, scale: string | undefined, denomination: string) => {
    ...
    found.push({ amount: amountOf(amount, scale), currency: moneyCurrency(denomination), quote: input.slice(index, end), index, end, notTotal: notTotal || /(?:на\s+каждого|с\s+человека)/u.test(context) });
  };
  ...
  for (const match of text.matchAll(new RegExp(`(?<!\\p{L})(?:бюджет(?:ом)?|бюджетім|бюджеті|максимум|потолок)\\s*[:—–=-]?\\s*(?:до\\s+)?(${number})\\s*(${unit})?(?![\\p{L}\\d])`, 'gu'))) put(match, match[1]!, match[2], '');
