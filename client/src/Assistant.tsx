...
export default function Assistant({ onActivity, onConfirm, onManualSearch, canSearch }: {
  onActivity: () => void;
  ...
  const reset = () => {
    cancel(); setMessages([]); setPrompt(''); setBrief(null); setError(''); setConfirmed(false); onActivity(); input.current?.focus();
  };
  const edit = (value: string) => {
    if (pending) cancel();
    setPrompt(value); setError(''); setConfirmed(false); onActivity();
  };
  const submit = async (event: FormEvent) => {
    ...
    cancel(); onActivity(); setError(''); setConfirmed(false); setPending(message);
  ...
  return <section className="assistant" aria-labelledby="assistant-title">
    ...
    <div className="assistant-review" aria-live="polite" aria-busy={!!pending}>
      {pending ? <div className="assistant-thinking" role="status"><span className="spinner" aria-hidden="true" /><h3>Разбираем вашу идею</h3><p>Выделяем условия и пожелания, проверяем, что нужно уточнить.</p></div> : brief ? <>
        ...
        {brief.warnings.length > 0 && <div className="brief-warnings">{brief.warnings.map((warning, index) => <p key={index}>{warning}</p>)}</div>}
